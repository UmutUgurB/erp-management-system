...
import { StockInFormData } from '@/types/inventory';
...
const stockInSchema = z.object({
  ...
  unitCost: z.number().min(0, 'Birim maliyet 0\'dan büyük olmalıdır').optional(),
...
export default function StockInForm({ onSuccess, onCancel }: StockInFormProps) {
  ...
  const generateReferenceNumber = () => {
    ...
    return `STK-IN-${timestamp}-${random}`;
  };
  ...
  const onSubmit = async (data: StockInFormData) => {
    try {
      ...
      showNotification({
        type: 'error',
        title: 'Hata',
        message: error.response?.data?.message || 'Stok girişi kaydedilirken hata oluştu'
      });
    } finally {
      setLoading(false);
    }
    ...
    <Card className="w-full max-w-2xl">
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {/* Birim Maliyet */}
          <div className="space-y-2">
            <Label htmlFor="unitCost">Birim Maliyet (Opsiyonel)</Label>
            <Input
              id="unitCost"
              type="number"
              step="0.01"
            ...
            {watchedUnitCost > 0 && watchedQuantity > 0 && (
              <p className="text-sm text-muted-foreground">
                Toplam Maliyet: ₺{(watchedUnitCost * watchedQuantity).toFixed(2)}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              id="reference"
              {...register('reference')}
              placeholder="Fatura, irsaliye vb."
            />
            {errors.reference && (
